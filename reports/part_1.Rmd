...
#boxplot
ggplot(data = db1) + 
  geom_boxplot(mapping = aes(x = factor(propclass), y = assessed_value))
...
mod_1 <- lm(sale_price ~ sale_date, data=db2)
...
lm_reg <- lm(sale_price ~ year_sale + factor(propclass) + assessed_value, data = db2)
...
lm_reg <- 
 linear_reg() %>%
  set_engine("lm") %>%
  fit(sale_price ~ year_sale+ factor(propclass) + assessed_value + tax_value, data = db2)
...
lm_fore <- 
 linear_reg() %>%
  set_engine("lm") %>%
  fit(`2015`~ assessed_value, data=db2)
...
lm_fore <- 
 linear_reg() %>%
  set_engine("lm") %>%
  fit(`2015`~ assessed_value + tax_value, data=db2)
...
lm_fore <- 
 linear_reg() %>%
  set_engine("lm") %>%
  fit(`2015`~ assessed_value + tax_value +sale_price, data=db2)
...
lm_fore <- lm(`2015` ~ year_sale + assessed_value + tax_falue + sale_price + factor(propclass), data = db2)
