...
mod_1 <- lm(sale_price ~ year_sale, data=db2)
...
#lm_reg %>% broom::augment() gives an error of Error in augment.model_fit(.) : 
  #argument "new_data" is missing, with no default

#(lm_reg %>% broom::augment(), aes(x=.resid)) +
  #geom_density(fill='navy', alpha=.6) +
   #scale_y_continuous(labels = function(x) format(x, scientific = FALSE)) +
  #scale_x_continuous(labels = function(x) format(x, scientific = FALSE))
...
lm_reg <- lm(sale_price ~ year_sale + factor(property_c) + assessed_value, data = db2)
...
lm_reg <- 
 linear_reg() %>%
  set_engine("lm") %>%
  fit(sale_price ~ year_sale + factor(property_c) + assessed_value + tax_value, data = db2)
...
lm_fore <- lm(`2015` ~ year_sale + assessed_value + tax_falue + sale_price + factor(property_c), data = db2)
