...
#trying a logistic regression not entirely sure of the interpretation but I belive glm does not return a r squared
...
lm_fore <- 
 linear_reg() %>%
  set_engine("lm") %>%
  fit(foreclose ~ assessed_value, data=db2)
...
lm_fore <- lm(`2015` ~ year_sale + assessed_value + tax_value + sale_price + factor(property_c), data = db2)

lm_fore %>% broom::tidy()
lm_fore %>% broom::glance()
lm_fore %>% broom::augment()

#augment works but no .resid column
#slight decrase of of r squared
